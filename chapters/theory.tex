...
\end{equation}
where $\theta_{W}$ is called the weak mixing angle and is defined as $\tan{\theta_{W}} = g'/g$. The field $\mathrm{Z}_{\mu}$ now picks up a mass term,
\begin{equation}
...
\end{figure}
In proton collisions at $\sqrt{s} = 13$\,TeV gluon fusion dominates over the other processes for a Higgs boson of mass $m_{H}=125$\,GeV with a cross section of 49\,pb. VBF is the second largest with 3.8\,pb, VH is third with 2.3\,pb and \ttH is last with $0.5$\,pb~\cite{HiggsPMs}. Although ggH has by far the largest crossection, the other production modes contain extra objects in their final state aiding the separation of Higgs bosons from events that resemble them. In particular VBF gives rise to two highly energetic jets from the final-state quarks with a large angular separation. 
